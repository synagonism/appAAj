...
/**
 * This class creates a TREE with all WHOLES of a concept.
 ...
 * @author HoKoNoUmo
 */
public class ViewAllWhoLevels extends JFrame
{
	static final long serialVersionUID = 21L;

	Vector<String> filesToSearch=new Vector<String>();		// Container for files-names to search for whos.
	Map<String,DefaultMutableTreeNode> whosFound=new HashMap<String,DefaultMutableTreeNode>();					// File-Names of ALL wholes.
  ...
  public ViewAllWhoLevels(String sbcpt)
  {
    ...
    pNorth.setBorder(BorderFactory.createEtchedBorder());
		JLabel label1 = new JLabel ("The TOTAL number of WHOLES is:  ");
    ...
    while(!filesToSearch.isEmpty())
		{
			...
			{
				JOptionPane.showMessageDialog(null, "ViewAllWhoLevels: Problem in " + currentFileName);
				continue;       // Try next file
			}
			Extract ex = new Extract();
			ex.extractWholes(currentFileName);
			Vector whos = ex.vectorWhoID;
			DefaultMutableTreeNode nodeToInsert = whosFound.get(currentFileName);
		...
		label2.setText(String.valueOf(whosFound.size()-1));
